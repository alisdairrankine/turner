...
func bigWorld() turner.Hitable {
	...
	//giant sphere to act as the ground
	scene = append(scene,
		geometry.Sphere{
			turner.Vec3{X: 0, Y: -1000, Z: 0},
			1000,
			material.Diffuse(
				texture.Checker(
					texture.Colour(turner.Vec3{X: 0, Y: 0, Z: 0}),
					texture.Colour(turner.Vec3{X: 0.9, Y: 0.9, Z: 0.9}),
				),
			),
		},
	)
	...
	scene = append(scene, geometry.Sphere{turner.Vec3{0, 1, 0}, 1, material.Dielectric(1.5)},
		...
		geometry.Sphere{turner.Vec3{4, 1, 0}, 1, material.Diffuse(texture.Colour(turner.Vec3{0.4, 0.2, 0.1}))})
